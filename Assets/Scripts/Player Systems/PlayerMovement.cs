...
public class PlayerMovement : MonoBehaviour
{
    ...
    [Header("Slope Collision Variables")]
    [SerializeField] private float _slopeRaycastLength;
    [SerializeField] private Vector3 _slopeRaycastOffset;
    ...
    private void Update()
    {
        ...
        if (Input.GetButtonDown("Jump") && !_isAttached)
        {
            _jumpBufferCounter = _jumpBufferLength; 
        }
        else if (Input.GetButtonDown("Jump") && _isAttached)
        {
        ...
        if (Input.GetKeyDown("w") && _isAttached) { Slide(1); }
        if (Input.GetKeyDown("s") && _isAttached) { Slide(-1); }
        Animation();
    }
    ...
    private void FixedUpdate()
    {
        ...
        if (!_isDashing)
        {
            ...
            if (_onSlope)
            {
                _rb.AddForce(Vector2.down * 7f, ForceMode2D.Force);
                _rb.gravityScale = 0;
                ApplyGroundLinearDrag();
                _extraJumpsValue = _extraJumps;
                _hangTimeCounter = _hangTime;
    ...
    public void Detach()
    {
        _hj.connectedBody.gameObject.GetComponent<RopeSegment>().isPLayerAttached= false;
        _isAttached = false;
        _hj.enabled = false;
    ...
    public void Slide(int direction)
    {
        ...
        if (direction > 0)
        {
            ...
            {
                if(myConnection.connectedAbove.gameObject.GetComponent<Rigidbody2D>() != null)
                {
    ...
    private void CheckCollisions()
    {
        ...
        //Slope Collisions
        _onSlope = Physics2D.Raycast(transform.position + _slopeRaycastOffset, Vector2.down, _slopeRaycastLength, _slopeLayer) ||
                    Physics2D.Raycast(transform.position - _slopeRaycastOffset, Vector2.down, _slopeRaycastLength, _slopeLayer);
    ...
    private void OnDrawGizmos()
    {
        ...
        //clope Check
        Gizmos.DrawLine(transform.position + _slopeRaycastOffset, transform.position + _slopeRaycastOffset + Vector3.down * _slopeRaycastLength);
        Gizmos.DrawLine(transform.position - _slopeRaycastOffset, transform.position - _groundRaycastOffset + Vector3.down * _slopeRaycastLength);
